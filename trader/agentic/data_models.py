...
class RelevancyDecision(BaseModel):
    ...
    is_relevant: bool = Field(..., description="Whether the post is relevant to the cryptocurrency.")
    reason: Optional[str] = Field(None, description="Short reason explaining the relevance decision.")
...
class SentimentResult(BaseModel):
    overall: Literal['Negative', 'Positive', 'Neutral'] = Field(..., description="The overall sentiment of the Cryptocurrency based on the received data.")
    score: float = Field(..., description="Sentiment score based on your analysis. Between 0 and 1.",)
    warnings: str = Field(..., description="Any additional considerances to bear in mind, such as potential warnings.")
...
class Tweet(BaseModel):
    ...
    username: Optional[str] = Field(None, description="Username (handle) of the tweet's author.")
    likes: Optional[int] = Field(None, description="Number of likes (if accessible).")
    retweets: Optional[int] = Field(None, description="Number of retweets (if accessible).")
...
class RedditPost(BaseModel):
    ...
    """
    title_plus_text: str = Field(..., description="Combination of the post title and body.")
    created_at: datetime = Field(..., description="UTC datetime when the post was created.")
    ...
    subreddit: Optional[str] = Field(None, description="Name of the subreddit (e.g. r/Crypto).")
...
class DiscordMessage(BaseModel):
    ...
    channel_id: str = Field(..., description="ID or name of the channel where this message was posted.")
    timestamp: datetime = Field(..., description="UTC time at which the message was sent.")
    mentions: List[str] = Field(default_factory=list, description="List of user mentions.")
    attachments: List[str] = Field(default_factory=list, description="List of attachment URLs, if any.")
...
class ConcatTweets(BaseModel):
    ...
    """
    ticker: str = Field(..., description="Symbol of the cryptocurrency, e.g. BTC or ETH.")
...
class SocialFeed(BaseModel):
    ...
    tweets: List[Tweet] = Field(default_factory=list, description="List of Tweet objects.")
    reddit_posts: List[RedditPost] = Field(default_factory=list, description="List of Reddit posts.")
    discord_messages: List[DiscordMessage] = Field(default_factory=list, description="List of Discord messages.")



class CultinessResult(BaseModel):
    score: float = Field(..., description="The overall cultiness score, between 0.0 and 1.0.")
...
class SentimentBreakdown(BaseModel):
    ...
    """
    tweets_sentiment: Optional[SentimentResult] = Field(None, description="Sentiment analysis result just for Tweets.")
    reddit_sentiment: Optional[SentimentResult] = Field(None, description="Sentiment analysis result just for Reddit.")
    discord_sentiment: Optional[SentimentResult] = Field(None, description="Sentiment analysis result just for Discord.")
    combined_sentiment: Optional[SentimentResult] = Field(None, description="An aggregated or averaged sentiment across all sources.")
...
class SentimentAgentInput(BaseModel):
    ...
    max_discord_messages: int = Field(10, description="Max Discord messages to retrieve from relevant channels.")
...
class SentimentAgentOutput(BaseModel):
    ...
    breakdown: SentimentBreakdown = Field(..., description="A breakdown of sentiment by source, plus combined.")
    generated_at: datetime = Field(default_factory=datetime.now, description="When the sentiment was generated (UTC).")
...
class TradeSignal(BaseModel):
    ...
    rationale: str = Field(..., description="Short explanation of why this action is recommended.")
