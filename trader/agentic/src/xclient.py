...
class XClient:
    ...
    def get_bearer_token(self, _api_key, _api_secret) -> str:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Error getting bearer token: {response.status_code} {response.text}"
            )
    ...
    def get_recent_tweets_with_backoff(
        self, query, hours=24, max_results=10, max_retries=5, backoff_factor=2
    ):
        """
        ...
        while attempt < max_retries:
            try:
                ...
                if attempt < max_retries:
                    ...
                    logging.warning(
                        f"[Retry {attempt}/{max_retries}] Error: {e}. "
...
if __name__ == "__main__":
    ...
    try:
        tweets_with_backoff = client.get_recent_tweets_with_backoff(
            ticker, max_results=5
        )
